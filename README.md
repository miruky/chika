...
冒険は一手ごとにブラウザ(localStorage)へ自動保存される。タブを閉じても、次に開いたときは同じ階・同じHP・同じ持ち物のまま続きから再開する。別シードの共有リンクを開いたときと、「新しい冒険」「この種でやり直す」を選んだときは、保存を捨てて新しい迷宮から始める。冒険が決着(生還・死亡)すると保存は消え、リロードしても終局画面を蒸し返さない。地形は持たずシードと階から作り直すため、保存はごく小さい。

### 戦績

決着すると、その冒険の到達階・ターン数・撃破数・レベルがバナーに出る。あわせて、これまでの最高到達階・生還回数・挑戦回数をブラウザに積み上げて表示する。記録はこの端末の中だけに残る。
...
`perform()` は行動がターンを消費したときだけ敵を動かす。壁に向かう・満タンで薬を飲むなど無効な手はターンを進めない。`serialize()` で途中状態を `GameSnapshot`(プレーンなJSON)へ書き出し、`Game.restore(snapshot)` で同じ局面を復元できる。地形はシードと階から作り直すので、保存に持つのは敵・床落ち・持ち物・探索済みだけで小さい。
...
- 自動保存はブラウザごとに1冒険ぶんで、複数の冒険を並行して保存する仕組みはない。別の端末へは引き継げない(シードを共有すれば同じ迷宮はやり直せる)。
